...
const host = "http://localhost:2555"; // Application host (NOT service host - that can be anything)
	...
	describe("Vacation Schedule Tests", () => {
		it("Load Vacation", async () => {
			...
			const list = await page.$$eval(
				`#confirmed-vacantions #list .container`,
		...
		it("Add Vacation", async () => {
			...
			const [name] = await page.$$eval(`#name`, (t) => t.map((s) => s.value));
		...
		it("Schedule Done", async () => {
			...
			await Promise.all([
				onResponse(),
				page.click(`#list .container .done-btn`),
			]);
